...
export default defineEventHandler(async (event) => {
  ...
  if (![15, 30, 60].includes(durationOption)) {
    throw createError({ statusCode: 400, message: 'durationOption must be 15, 30, or 60' })
  }
  ...
  if (profile && profile.videos_processed_this_month >= profile.monthly_video_limit) {
    throw createError({ statusCode: 429, message: 'Monthly video limit reached. Please upgrade your plan.' })
  }
